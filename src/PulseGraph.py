...
# TODO: Add B signal, phased
# TODO: Make the signal muteable
# TODO: Add legend
# TODO: Add axes text
# TODO: Add Tolerances / Derating?

# Set up plot
plot = figure(plot_height=400, plot_width=800, title="Rotary Encoder Signals",
...
plot.line('x', 'y', source=source, line_width=1, line_alpha=0.6)  # pylint: disable=too-many-function-args
plot.line('x', 'y', source=source2, line_width=2, line_alpha=0.6) # pylint: disable=too-many-function-args
plot.add_layout(hlineHigh)
plot.add_layout(hlineLow)


# Set up widgets
voltageLow = TextInput(title="Low Logic Voltage Threshold (V)", value=str(VTHLOW), width=200)
voltageHigh= TextInput(title="High Logic Voltage Threshold (V)", value=str(VTHHIGH), width=200)
...
detents = Slider(title="Detents", value = DETENTS, start=1, end=30, step=1, width=275)
pulsesPerDetent = Slider(title="Pulses per Detent", value=PULSES_PER_DETENT, start=1, end=2, step=1, width=115)
...
def update_data(attrname, old, new):
    ...
    thresholdHigh = float(voltageHigh.value)
    thresholdLow = float(voltageLow.value)
    ...
    hlineHigh.location = thresholdHigh
    hlineLow.location = thresholdLow

    plot.x_range.end = duration

# for w in [offset, amplitude, phase, freq, detents, pulsesPerDetent]:
for w in [voltage, voltageLow, voltageHigh, resistance, capacitance, detents, pulsesPerDetent, offset, phase]:
    w.on_change('value', update_data)


# Set up layouts and add to document
# inputs = column(text, offset, amplitude, phase, freq, detents, pulsesPerDetent)
inputs = column(voltage, row(voltageLow, voltageHigh), resistance, capacitance, row(detents, pulsesPerDetent), offset, phase)
